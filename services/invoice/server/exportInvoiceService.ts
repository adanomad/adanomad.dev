...
export async function exportInvoiceService(req: NextRequest) {
  ...
  try {
    switch (format) {
      case ExportTypes.KV:
        // Save the invoice to the KV store
        const filename = req.nextUrl.searchParams.get("filename");
        const username = req.nextUrl.searchParams.get("username");
        if (!filename || !username) {
          throw new Error("Filename and username are required");
        }
        await upsertInvoice(username, body.id, filename, body);
        return new NextResponse(
          JSON.stringify({
            message: "Invoice saved to KV",
            invoiceId: body.id,
          }),
          {
            status: 200,
